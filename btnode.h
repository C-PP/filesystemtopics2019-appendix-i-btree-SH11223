...
public:
	...
	int Copy(int recAddr, BTreeNode<keyType>* newNode);
	// move keys into newNode
...
template <class keyType>
int BTreeNode<keyType>::Copy(int recAddr, BTreeNode<keyType> * newNode)
{
	// check for sufficient number of keys
	newNode->NumKeys = NumKeys;
	newNode->MaxBKeys = MaxBKeys;
	newNode->MaxKeys = MaxKeys;
	newNode->RecAddr = recAddr;
	for (int i = 0; i < this->NumKeys; i++)
	{
		newNode->Keys[i] = this->Keys[i];
		newNode->RecAddrs[i] = RecAddrs[i];
	}
	// set number of keys in the two Nodes
